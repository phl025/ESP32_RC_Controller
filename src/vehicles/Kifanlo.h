...
volatile int fullThrottleVolumePercentage = 100; // Volume Percentage while full throttle (for rev sound as well)
#include "sounds/Kifanlo_ralenti_3.h"
...
volatile const uint16_t idleEndPoint = 450;     // above this point, we have 100% rev and 0% idle sound volume (usually 500, min. 50 more than revSwitchPoint)
...
#ifdef REV_SOUND
#include "sounds/Kifanlo_avance_3B.h"
#endif
...
volatile int brakeVolumePercentage = 0; // Adjust the brake volume (usually = 200%)
...
volatile int sound1VolumePercentage = 80; // Adjust the sound1 volume (usually = 100%)
#include "sounds/aux_sounds/sf_mouettes_01_22k_9db.h"     // Mouettes
...
// Choose the additional "sound2" - Add PHL
#define SOUND2
#ifdef SOUND2
volatile int sound2VolumePercentage = 40;       // Adjust the sound volume (usually = 100%)
//#include "sounds/aux_sounds/winch_22k.h"      // Winch
#include "sounds/aux_sounds/winch_22k_3db.h"    // Winch
#endif
...
// Choose boat specific options  -----------------------------------------------------------------------------------------
#define BOAT_MODE               // For Boat, no gear (use escRampTimeSecondGear)

// Acceleration & deceleration settings (BOAT_MODE) ----------------------------------------------------------------------
const uint8_t escRampTimeMs = 20;           // Time base for ESP management (ms) (BOAT_MODE)
const uint8_t escRampAcc = 8;              // Acceleration rampe (µs), ~ 450 / escRampAcc * escRampTimeMs, Example = 450/9*20 -> 1s
const uint8_t escRampDec = 8;              // Decceleration rampe (µs), ~ 450 / escRampDec * escRampTimeMs, Example = 450/9*20 -> 1s
...
// Clutch parameters (about 90 for manual transmission, 10 for automatic, 500 to desable) -----------------------------------------------
uint16_t clutchEngagingPoint = 500; //90; // CEP. The "clutch" is engaging above this point = engine rpm sound in synch with ESC power

// Engine parameters ----------------------------------------------------------------------------------------------------
// Engine max. RPM in % of idle RPM. About 200% for big Diesels, 400% for fast running motors.
uint32_t MAX_RPM_PERCENTAGE = 170; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage!

// Engine mass simulation
//#define SUPER_SLOW // super slow running, heavy engine for locomotives etc. Makes rpm response much slower
// timebase = 2ms, 5ms if SUPER_SLOW
