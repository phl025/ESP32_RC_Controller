...
// AlfredoCRSF.cpp : 
//
// Version modified by PHL 09-2025
//
...
{
    if (_linkIsUp && millis() - _lastChannelsPacket > CRSFFAILSAFE_STAGE1_MS)
    {
        _linkIsUp = false;
    }
}
...
void AlfredoCRSF::packetChannelsPacked(const crsf_header_t *p)
{
    ...
    _newChannelsPacket = true;  // Add PHL
}
...
/// @brief newChannelsPacket : Added by PHL
/// @return 
bool AlfredoCRSF::newChannelsPacket()
{
    if (_newChannelsPacket)
    {
        _newChannelsPacket = false;
        return true;
    }
    else    
        return false;
